...
prev.prev - 上上步（支持无限嵌套）`,
  ...
  linker: {
    ...
    instructions: {
      idle: '点击"开始连线"以连接外部象棋软件。程序会自动截取目标窗口，识别棋盘位置并同步局面。在自动落子模式下，会自动在外部软件中走棋。',
      selecting: '请从下方列表中选择要连线的象棋软件窗口，然后点击"连接"。',
      connected:
        '连线中...正在监控棋盘变化。检测到对手走棋后会自动应用到本地，并在自动模式下将引擎最佳着法发送到外部软件。',
      paused: '连线已暂停。点击"继续"恢复监控。',
    },
    error: {
      modelInitFailed: '图像识别模型初始化失败',
      noBoardDetected: '未检测到棋盘',
      recognitionFailed: '识别失败',
      listWindowsFailed: '获取窗口列表失败',
      windowNotFound: '未找到指定窗口',
      noWindowSelected: '请先选择一个窗口',
    },
    settingsForm: {
