...
prev.prev - 上上步（支援無限巢狀）`,
  ...
  linker: {
    ...
    instructions: {
      idle: '點擊「開始連線」以連接外部象棋軟體。程式會自動截取目標視窗，識別棋盤位置並同步局面。在自動落子模式下，會自動在外部軟體中走棋。',
      selecting: '請從下方列表中選擇要連線的象棋軟體視窗，然後點擊「連接」。',
      connected:
        '連線中...正在監控棋盤變化。檢測到對手走棋後會自動應用到本地，並在自動模式下將引擎最佳著法發送到外部軟體。',
      paused: '連線已暫停。點擊「繼續」恢復監控。',
    },
    error: {
      modelInitFailed: '圖像識別模型初始化失敗',
      noBoardDetected: '未檢測到棋盤',
      recognitionFailed: '識別失敗',
      listWindowsFailed: '獲取視窗列表失敗',
      windowNotFound: '未找到指定視窗',
      noWindowSelected: '請先選擇一個視窗',
    },
    settingsForm: {
